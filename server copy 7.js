            ...
            try {
                ...
                if (matchFound) {
                    ...
                    if (parentSlide) {
                        ...
                        const mockupStyleDiv = await parentSlide.$('.mockup-style.mockup-slide');
                        if (mockupStyleDiv) {
                            try {
...
async function deleteTemplate(page) {
    try {
        ...
        const dropdownToggle = await page.waitForSelector(SELECTORS.TEMPLATE_VIEW.DROPDOWN_TOGGLE, {
            visible: true,
            timeout: 5000
        });
        ...
        await page.evaluate(() => {
            const dropdownToggleElement = document.querySelector('a[data-test="dPfKn2n8wXKSCJw"]');
            if (dropdownToggleElement) {
                const dropdownContainer = dropdownToggleElement.closest('.pf-dropdown.dropdown-button');
                if (dropdownContainer) {
                    // Add the required classes to keep the dropdown open
                    dropdownContainer.className = 'pf-dropdown dropdown-button dropdown open';
                }
            }
        });
        ...
        await page.evaluate(() => {
            const dropdownContainer = document.querySelector('.pf-dropdown.dropdown-button');
            if (dropdownContainer) {
                dropdownContainer.className = 'pf-dropdown dropdown-button dropdown open';
            }
        });
        ...
        const deleteButton = await page.waitForSelector(SELECTORS.TEMPLATE_VIEW.DELETE_BUTTON, {
            visible: true,
            timeout: 5000
        });
